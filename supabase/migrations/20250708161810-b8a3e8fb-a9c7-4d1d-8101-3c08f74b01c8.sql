...
BEGIN
    ...
    LOOP
        -- Get all accounts for this user in an array
        SELECT ARRAY(
            SELECT id FROM paper_trading_accounts 
            WHERE user_id = user_rec.user_id 
            ORDER BY created_at
        ) INTO user_accounts;
        
        account_count := array_length(user_accounts, 1);
        
        -- Skip if user has no accounts
        IF account_count IS NULL OR account_count = 0 THEN
            CONTINUE;
        END IF;
        ...
        LOOP
            ...
            account_index := (account_index % account_count) + 1;
        ...
        RAISE NOTICE 'Updated trades for user % across % accounts', user_rec.user_id, account_count;
    END LOOP;
END $$;